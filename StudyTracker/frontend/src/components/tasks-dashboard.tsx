...
export default function TasksDashboard() {
  ...
  const getTopTasks = (count: number) => {
    return tasks
      .filter((task) => {
        // Apply type filter
        const typeMatch =
          activeTypeFilters.length === 0 ||
          activeTypeFilters.includes(task.type);
        // Apply state filter
        const stateMatch =
          (!task.completed &&
  ...
  const renderTask = (task: Task) => {
    ...
    return (
      <Card key={task._id?.toString()} className="w-full">
        <CardHeader
          className="p-4 cursor-pointer"
          onClick={() => setExpandedTaskId(isExpanded ? null : task._id ?? null)}
        >
